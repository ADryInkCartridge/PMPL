...
<div class="p-10">
    <div class="font-medium text-white text-3xl pb-4">Edit Tahap</div>
    <div class="w-full tambahuserheight bg-white rounded-xl p-10 relative">
        @include('flash-message')
        <form class="flex flex-col gap-y-8  h-full" method="POST" action="{{ route('tahap.update') }}">
            @csrf
            <input type="hidden" id="id" name="id" value="{{$tahap->id}}">
